  ...
  while ( k < iData) {
    ...
    for (n=0; n<m; n++) {
      ...
      if (   type > 0  /*&& index != reftype*/ )  {
        ...
        hrt[n] = 1000 + DataArray[k+n].time - evt_start + DataArray[k+n].cfdTime;
        // std::cout<<1000 + DataArray[k+n].time - evt_start<<", "<<DataArray[k+n].cfdTime<<std::endl;
        ...
        trace[type][index] = (std::vector<unsigned int>)DataArray[k+n].trace;
       ...
       index = ntmc[type][DataArray[k+n].modnum][DataArray[k+n].chnum];
            //~if (index != reftype) {
              ...
              for(int i = 0; i < trace[type][index].size(); i++){
                ...
                TRACE_branch[type][index-1].push_back((int)trace[type][index][i]);
                //at high rates, this can lead to having multiple independent traces plotted after each other
              }
    ...
    TRACE_branch[type][index-1].clear();
    
    // std::cout<<"trace = "<<sizeof(trace)<<", TRACE_branch = "<<sizeof(TRACE_branch)<<", data array trace = "<<sizeof(DataArray[k+n].trace)<<
    // ", data array [k+n] = "<<sizeof(DataArray[k+n])<<" , Data array total = "<<sizeof(DataArray)<<" , tree total = "<<sizeof(tree)<<std::endl;
	  // std::cout<<TRACE_branch[1][0][23]<<","<<std::endl;
    // std::cout<<"size =" <<get(trace)<<std::endl;
    iEvt++;
    k+=m;
